...
#### Key Trigger Implementation Examples
The following SQL definitions illustrate the system's approach to server-side logic, specifically enabling the "Appointments" and "Main" module functionalities:

**1. Role Verification (`trg_ValidatePhysicianAssignment`)**
*Prevents scheduling errors by ensuring the assigned staff member possesses the 'Physician' role before an appointment is created.*
```sql
DELIMITER //
CREATE TRIGGER trg_ValidatePhysicianAssignment
BEFORE INSERT ON Appointment
FOR EACH ROW
BEGIN
    DECLARE role_title VARCHAR(50);

    -- Fetch the role of the assigned physician
    SELECT r.Title INTO role_title
    FROM Employee e
    JOIN Role r ON e.Role_ID = r.Role_ID
    WHERE e.Employee_ID = NEW.Physician_ID;

    -- Enforce strict role validation
    IF role_title != 'Physician' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Integrity Violation: Assigned user is not a Physician.';
    END IF;
END; //
DELIMITER ;
```

**2. Automated Billing Synchronization (`trg_SyncAppointmentBilling`)**
*Automates the financial lifecycle by generating a pending invoice immediately upon appointment confirmation, ensuring no billable event is missed.*
```sql
DELIMITER //
CREATE TRIGGER trg_SyncAppointmentBilling
AFTER UPDATE ON Appointment
FOR EACH ROW
BEGIN
    -- Check if status changed to 'Confirmed' (ID: 1)
    IF NEW.Status_ID = 1 AND OLD.Status_ID != 1 THEN
        INSERT INTO billing_invoice (Appointment_ID, Amount, Status, Created_At)
        VALUES (NEW.Appointment_ID, 0.00, 'Pending', NOW());
    END IF;
END; //
DELIMITER ;
```
